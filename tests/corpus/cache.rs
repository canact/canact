...
#[test]
fn cache_key_format_is_model_provider_unset_v50() {
    assert_eq!(PROBE_SUITE_VERSION, 50);
    ...
    assert_eq!(k, "model|provider|unset|v50|full|novision");
}
...
fn get_misses_when_suite_differs() {
    ...
    assert!(
        cache
            ...
            .is_none(),
        "suite v50 must not hit v6 cache entry"
    );
}
