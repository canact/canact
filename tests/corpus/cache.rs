...
#[test]
fn cache_key_format_is_model_provider_unset_v39() {
    assert_eq!(PROBE_SUITE_VERSION, 39);
    ...
    assert_eq!(k, "model|provider|unset|v39|full|novision");
}
...
fn get_misses_when_suite_differs() {
    ...
    assert!(
        cache
            ...
            .is_none(),
        "suite v39 must not hit v6 cache entry"
    );
}
